...
import com.yaoguang.lib.annotation.aspect.MemoryCache;
...
public class InstanceProcessor {
    public void process(ProcessingEnvironment processingEnv, RoundEnvironment roundEnv, AbstractProcessor mAbstractProcessor, Class<? extends Annotation> annotation) {
        ...
        try {
            for (TypeElement element : ElementFilter.typesIn(roundEnv.getElementsAnnotatedWith(InstanceFactory.class))) {
                ...
                if (!Utils.isValidClass(processingEnv.getMessager(), element)) return;
