...
import com.yaoguang.apt.util.Utils;
...
public class ApiFactoryProcessor {
    ...
    public void process(ProcessingEnvironment processingEnv, RoundEnvironment roundEnv, AbstractProcessor mAbstractProcessor, Class<? extends Annotation> annotation, String api, String className) {
        ...
        try {
                    ...
                    if (TypeName.get(executableElement.getReturnType()).toString().contains(DATA_ARR_CLASS)) {//返回列表数据
                        ...
                        methodBuilder.addStatement(
                                "return $T.getInstance()" +
                                        ".service.$L($L)" +
                                        ".compose($T.io_main())"
                                , ClassName.get("com.yaoguang.driver.net", api)
                                , e.getSimpleName().toString()
                                , blockBuilder.build().toString()
                                , ClassName.get("com.yaoguang.common.common", "RxSchedulers"));
            ...
            JavaFile javaFile = JavaFile.builder(Utils.AppServerPackageName, tb.build()).build();// 生成源代码
