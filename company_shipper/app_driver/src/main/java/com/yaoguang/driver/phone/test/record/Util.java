package com.yaoguang.driver.phone.test.record;
...
public class Util {
	...
	public static List<TraceLocation> parseTraceLocationList2(
			List<Location> list) {
		List<TraceLocation> traceList = new ArrayList<TraceLocation>();
		if (list == null) {
			return traceList;
		...
		for (int i = 0; i < list.size(); i++) {
			...
			location.setBearing(ObjectUtils.parseFloat(amapLocation.getAzimuth()));
			location.setLatitude(amapLocation.getLat());
			location.setLongitude(amapLocation.getLon());
			location.setSpeed(ObjectUtils.parseFloat(amapLocation.getSpeed()));
			location.setTime(DateUtils.stringToDate(amapLocation.getPositionTime(),DateUtils.YYYY_MM_DD_HH_MM_SS).getTime());
			traceList.add(location);
		}
		return traceList;
	}
	...
	/**
	 * Location List 转为LatLng list
	 * @param list
	 * @return
	 */
	public static List<LatLng> parseLatLngList2(List<Location> list) {
		List<LatLng> traceList = new ArrayList<LatLng>();
		if (list == null) {
			return traceList;
		}
