...
//			AMapLocation endLoc = mRecord.getEndpoint();
            LatLng startLatLng = new LatLng(locations.get(0).getLat(),
                    locations.get(0).getLon());
            LatLng endLatLng = new LatLng(locations.get(locations.size() - 1).getLat(),
                    locations.get(locations.size() - 1).getLon());
            mOriginLatLngList = Util.parseLatLngList2(locations);
    ...
    public void onRequestFailed(int arg0, String arg1) {
        Toast.makeText(BaseApplication.getInstance(), "轨迹纠偏失败:" + arg1,
