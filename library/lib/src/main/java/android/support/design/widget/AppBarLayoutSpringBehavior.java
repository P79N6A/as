...
public class AppBarLayoutSpringBehavior extends AppBarLayout.Behavior {
    ...
    private static final String TAG_TOOLBAR_BACKGROUND = "toolbarBackground";
    ...
    public boolean onLayoutChild(CoordinatorLayout parent, AppBarLayout abl, int layoutDirection) {
        ...
        if (mToolBar != null) {
            abl.addOnOffsetChangedListener((appBarLayout, verticalOffset) -> {
                View view = mToolBar.findViewWithTag(TAG_TOOLBAR_BACKGROUND);
                if (view != null)
                    view.setAlpha((float) Math.abs(verticalOffset) / (float) appBarLayout.getTotalScrollRange());
            });
        }
        return handled;
    }
    ...
    private void animateFlingSpring(final CoordinatorLayout coordinatorLayout, final AppBarLayout abl, int originNew) {
        if (mFlingAnimator == null) {
            ...
            mFlingAnimator.addUpdateListener(animation -> updateSpringHeaderHeight(coordinatorLayout, abl, (int) animation.getAnimatedValue()));
    ...
    private void animateRecoverBySpring(final CoordinatorLayout coordinatorLayout, final AppBarLayout abl) {
        if (mSpringRecoverAnimator == null) {
            ...
            mSpringRecoverAnimator.addUpdateListener(animation -> updateSpringHeaderHeight(coordinatorLayout, abl, (int) animation.getAnimatedValue()));
        } else {
