package com.yaoguang.widget.textview;
 ...
 * https://github.com/LanceCong/Android-RunningTextView
 * @author Lance
 *
 */
...
//		formater = new DecimalFormat("00.00");// 最多两位小数，而且不够两位整数用0占位。可以通过setFormat再次设置
		formater = new DecimalFormat("0");// 最多两位小数，而且不够两位整数用0占位。可以通过setFormat再次设置
		handler = new Handler() {
			@Override
...
//				Log.v("nowNumber增加之后的值", nowNumber + "");
				...
				if (nowNumber < content) {
					...
					RunningTextView.this.setText(formater.format(content));// 最后显示的数字，动画停止
				}
			}
		};
	}
