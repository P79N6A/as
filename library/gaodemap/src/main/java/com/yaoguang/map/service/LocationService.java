package com.yaoguang.map.service;
...
import com.yaoguang.map.NotiService;
...
public class LocationService extends NotiService {
    ...
    void startLocation() {
        ...
        // 地址信息 返逆推，也就是不只返回坐标，还有速度，角度，中文地址等等
...
//                sb.append("定位失败：location is null!!!!!!!");
                ...
                Intent mIntent = new Intent(Constant.RECEIVER_ACTION);
                mIntent.putExtra("longitude", aMapLocation.getLongitude());
                mIntent.putExtra("latitude", aMapLocation.getLatitude());
                mIntent.putExtra("time", Utils.formatUTC(aMapLocation.getTime(), "yyyy-MM-dd HH:mm:ss"));
                mIntent.putExtra("azimuth", aMapLocation.getBearing());
                mIntent.putExtra("speed", aMapLocation.getSpeed());
                mIntent.putExtra("count", locationCount);
                if (locationCount == 100) {
                    // 当次数达到100的时候，将重置，为了每100次，传输数据到服务器
                    locationCount = 0;
                }
                //发送广播
