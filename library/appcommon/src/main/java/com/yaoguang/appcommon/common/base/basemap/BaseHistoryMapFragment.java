...
public abstract class BaseHistoryMapFragment extends BaseMapDataBindFragment {
    // 轨迹缓存
     ...
     * 作    者：韦理英
     * 时    间：
     * @param readHistoryLatLngs
     */
    public void startCarHistory(List<LatLng> readHistoryLatLngs) {
        ULog.i("startCarHistory");
        // 轨迹是空的或轨迹没数据就不放
        if (isNull(readHistoryLatLngs) || mAMap == null) return;
        ...
        if (readHistoryLatLngs.size() > 3) {
            try {
                ...
                movePoint(points.get(local),true);
                zoom(10f);
            }
        } else { // 如果轨迹小于3就不要bounds，直接移动到起点
            movePoint(points.get(0),true);
        }
...
//            }
    ...
    private boolean isNull(List<?> positionLogs) {
        if (positionLogs == null || positionLogs.isEmpty()) return true;
        return false;
    }
}
